...
export default function DailyCalendarClient({ date, initialEvents, tags }: DailyCalendarClientProps) {
  ...
  // anchor and is never mutated after being set — the overlay is positioned
  // imperatively from it (see positionOverlay) so it tracks the event with no
  ...
  const { activeDate, navigateTo } = useDateNavigation(date, '/calendar');
